...
/**
 * FIXED VERSION: Works on Linux (Render) without hardcoded paths
 * Uses Puppeteer's built-in Chromium with proper memory management
 */
async function generatePdfFromHtml(htmlContent) {
  let browser;
  try {
    ...
    // Launch with proper Linux/Render configuration
    browser = await puppeteer.launch({
      headless: "new",
      args: [
        "--no-sandbox",
        "--disable-setuid-sandbox",
        "--disable-dev-shm-usage", // Critical for Render's limited memory
        "--disable-gpu",
...
/**
 * Generate optimized resume PDF from user data
 * Uses Google Gemini AI to create tailored resume HTML
 */
async function generateResumePdf({ resume, selfDescription, jobDescription }) {
  try {
    const prompt = `
...
`;
    ...
    for (let i = 0; i < 2; i++) {
      try {
        ...
        if (data.html && data.html.includes("<html")) {
          html = data.html;
          console.log("Valid HTML received from AI");
          break;
        }
        ...
        if (i === 1) {
          throw new Error("AI failed to generate resume HTML after 2 attempts");
        }
      }
    }

    if (!html) {
      throw new Error("Invalid HTML from AI - missing or malformed HTML content");
    }

    console.log("Starting PDF conversion from HTML...");
    const pdfBuffer = await generatePdfFromHtml(html);
